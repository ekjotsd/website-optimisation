...
var resizePizzas = function(size) { 
  ...
  function changePizzaSizes(size) {
    ...
    // Sets randomPizzaContainer
    var randomPizzaContainer = document.getElementsByClassName("randomPizzaContainer");
    var newwidth = [];
    for (var n = 0; n < pizzaContainerLength; n++) {
      var dx = determineDx(randomPizzaContainer[n], size);
    ...
    for (var cl = 0; cl< pizzaContainerLength; cl++) {
...
// making pizzasDiv out of loop to prevent repeated calls
var pizzasDiv = document.getElementById("randomPizzas");
for (var ac = 2; ac < 100; ac++) {
...
function logAverageFrame(times) {   // times is the array of User Timing measurements from updatePositions()
  ...
  var sum = 0;
  for (var l = numberOfEntries - 1; l > numberOfEntries - 11; l--) {
    sum = sum + times[l].duration;
  }
...
function updatePositions() {
  ...
  var stuffy = document.body.scrollTop / 1250;
  for (var m = 0; m < items.length; m++) {
    var phase = Math.sin((stuffy) + (m % 5));
